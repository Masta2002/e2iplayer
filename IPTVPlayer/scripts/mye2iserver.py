...
UPDATE_URL = 'http://www.e2iplayer.gitlab.io/mye2iv3_1.17.zip'  # NOSONAR - host's cert doesn't cover this name, https fails SNI check
...
def redirect_handler_factory(url):
    ...
    if 'e2itcf' in url:
        ...
        job_type = 'captcha'
    min_version_str = '.'.join(str(p) for p in MIN_EXTENSION_VERSION)
    to_write = '''
<!doctype html>
<html>
    <body style="background-color: #c8dee1">
        <div id="mye2i_%s" style="background:#c0392b;color:#fff;padding:14px;text-align:center;font-family:sans-serif;font-size:16px;">
            Deine MyE2i-Erweiterung ist veraltet oder unbekannt (mindestens v%s benoetigt). Bitte aktualisieren:
            <a href="%s" style="color:#fff;text-decoration:underline;">Download neue Version</a>
        </div>
        ...
        def do_GET(self):
            ...
            if route == '/':
                ...
                if not ext_version:
                    ...
                    print('[MyE2i-Browser] Extension version check: v%s is OUTDATED (min v%s)' % (ext_version, min_version_str))
                    updateStatus('status', 'MyE2i-Erweiterung veraltet (v%s) - bitte aktualisieren: %s' % (ext_version, UPDATE_URL))
                else:
                    print('[MyE2i-Browser] Extension version check: v%s OK (min v%s)' % (ext_version, min_version_str))
                sys.stdout.flush()
                return
